...
while True:
    ...
    for sensor in sensors:
        ...
        for i in range(0, 1):
            ...
            if (distance > radius and distance < (radius+90)):
                print "Found something! Call that goddamn function!",
                process_date = subprocess.Popen("date +\"%M%S\"".split(), stdout=subprocess.PIPE)
                output_date, error_date = process_date.communicate() 
                image_name = "/home/pi/Desktop/Identificador_de_objetos/camera_shot/image" + output_date.rstrip() + ".jpg"
                os.system("raspistill -t 280 -o " + image_name)
